...
def resolve_active_profile():
    ...
    def setting(name):return (os.getenv(name,values.get(name) or '') or '').strip().strip('"').strip("'")
    address,ca=setting('REMOTERF_ADDR'),setting('REMOTERF_CA_CERT')
    if not address or not ca:return None
    return DirectConnectionProfile(address,setting('REMOTERF_TLS_SERVER_NAME') or None,Path(ca).expanduser())
